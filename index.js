...
async function ensureDatabaseExists() {
  const targetDB = process.env.DB_NAME || "majds_macaroon_market";
  const dbConfig = {
...
const AppDataSource = new DataSource({
  ...
  password: process.env.DB_PASS || "postgres",
  database: process.env.DB_NAME || "majds_macaroon_market",
...
app.get("/", (req, res) => {
      ...
      ">
        <div class="d-flex h-100 align-items-center justify-content-center">
          <div class="text-center text-white">
            <h1 class="display-3">Welcome to Majd's Macaroon Market!</h1>
  ...
  `;
  res.send(renderPage("Majd's Macaroon Market", content));
});
...
app.get("/products", async (req, res) => {
  try {
    ...
    `;
    res.send(renderPage("Products - Majd's Macaroon Market", html));
  } catch (error) {
...
app.get("/cart", (req, res) => {
  ...
  `;
  res.send(renderPage("Your Cart - Majd's Macaroon Market", content));
});
...
app.get("/checkout", (req, res) => {
  ...
  `;
  res.send(renderPage("Checkout - Majd's Macaroon Market", content));
});
...
app.post("/checkout", async (req, res) => {
  ...
  try {
    ...
    `;
    res.send(renderPage("Order Confirmation - Majd's Macaroon Market", content));
  } catch (error) {
